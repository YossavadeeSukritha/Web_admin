...
// Endpoint for clock-in with geolocation validation
app.post('/api/clockin', async (req, res) => {
    const { employee_id, latitude, longitude } = req.body;

    if (!employee_id || !latitude || !longitude) {
        return res.status(400).json({ message: 'Missing required data (employee_id, latitude, longitude).' });
    }

    try {
        // Fetch employee's assigned location and default radius
        const locationQuery = `
            SELECT 
                l.latitude, 
                l.longitude, 
                COALESCE(s.setting_value::numeric, 100) AS radius -- Default radius 100 meters
            FROM EmployeeLocations el
            JOIN Locations l ON el.location_id = l.location_id
            LEFT JOIN Settings s ON s.setting_key = 'default_radius'
            WHERE el.employee_id = $1
        `;
        const locationResult = await pool.query(locationQuery, [employee_id]);

        if (locationResult.rows.length === 0) {
            return res.status(404).json({ message: 'No assigned location found for this employee.' });
        }

        const assignedLocation = locationResult.rows[0];
        ...
        if (distance > assignedLocation.radius) {
            return res.status(403).json({
                message: 'Clock-in failed. You are outside the allowed radius.',
                distance: `${distance.toFixed(2)} meters`,
                allowed_radius: `${assignedLocation.radius} meters`
            });
        }

        // Record the clock-in data
        const clockInQuery = `
            INSERT INTO ClockInOut (employee_id, start_time, time_in_location, location_id, status)
            VALUES ($1, NOW(), $2, $3, $4)
            RETURNING *
        `;
        const clockInResult = await pool.query(clockInQuery, [
            employee_id,
            distance <= assignedLocation.radius ? 'ON SITE' : 'OFF SITE',
            assignedLocation.location_id,
            'On-time'
        ]);

        res.status(200).json({
            message: 'Clock-in successful!',
            clockIn: clockInResult.rows[0]
        });
    } catch (error) {
